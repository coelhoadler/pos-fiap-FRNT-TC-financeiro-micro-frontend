...
import { LoginMenuItems, LoginLogadoMenuItems } from "./menuItems";
  ...
  return (
    <header className="bg-black min-h-[80px] max-md:min-h-[56px] flex items-center px-10 fixed w-full z-10">
      <div className="max-md:hidden w-full">
        <MenuDesktop variant="login" menuLinksItemsLogado={LoginLogadoMenuItems} menuLinksItems={LoginMenuItems} useAuth={userInfo} />
      </div>
      <div className="max-md:block hidden w-full">
        <MenuMobile variant="login" menuLinksItemsLogado={LoginLogadoMenuItems} menuLinksItems={LoginMenuItems} useAuth={userInfo} />
      </div>
    </header>
  );
};
